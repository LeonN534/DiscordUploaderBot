const GREETING_MESSAGE = `Hello!
Thanks for adding me to your server!
In order to be able to upload to start uploading files to your VPS you need to set me up first.
Here are the commands you need to set before using me:`;

const SET_CHANNEL_COMMAND = `!set channel`;
const SET_SERVER_IP_COMMAND = `!set server ip`;
const SET_SERVER_PORT_COMMAND = `set server port`;
const SET_SERVER_PASSWORD_COMMAND = `!set server password`;

export {GREETING_MESSAGE, SET_CHANNEL_COMMAND, SET_SERVER_IP_COMMAND, SET_SERVER_PASSWORD_COMMAND, SET_SERVER_PORT_COMMAND};

